...
| **N5** | `STATUS[24:20] RX_FIFO_LVL` — "how many valid 32-Bit words in the Rx FIFO, max. is 31". | `XDCFG_STATUS` | The 202-word destination is **6.5×** the entire RxFIFO. C2's bandwidth warning is not theoretical here. **How it is reflected (clarified 2026-08-29 after D2 review):** not as a runtime read — a UART-driven `md` cannot sample a FIFO that fills in microseconds, and after completion the level is 0 by construction — but as policy: `RX_FIFO_OV` (N6) is a *device-state* stop with **no retry** (§7.1, §7.4), the timeout is derived for the full-rate case (U2), and the two-unidirectional/one-bidirectional question carries a named FIFO hypothesis (`s0_derived_sequence.md` §8a, H-FIFO). |
